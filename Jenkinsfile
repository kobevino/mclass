pipeline {
    ...
    environment {
        ...
        CONTAINER_NAME = "springbook-container" // 도커 컨테이너 이름
        JAR_FILE_NAME = "app.jar" // 복사할 JAR 파일 이름
        PORT = "8081"  // 컨테이너와 연결할 포트

        REMOTE_USER = "ec2-user" // 원격(spring) 서버 사용자
        REMOTE_HOST = "15.165.231.53" // 원격(spring) 서버 IP(Public IP)

        REMOTE_DIR = "/home/ec2-user/deploy" // 원격 서버에 파일 복사할 경로
        SSH_CREDENTIALS_ID = "2e016c96-2e98-410d-824a-3e586d9c1f62" // Jenkins SSH 자격 증명 ID
    }
